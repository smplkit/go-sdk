...
// --- ConfigRuntime shared WS paths ---

func TestConfigRuntime_WsLoop_SharedWS(t *testing.T) {
	ws := newSharedWebSocket("https://app.smplkit.com", "test")
	...
	// Wait for it to register as connected
	time.Sleep(50 * time.Millisecond)
	assert.Equal(t, "connected", rt.ConnectionStatus())

	// Close should exit the loop
	close(rt.closeCh)
	<-rt.wsDone
	assert.Equal(t, "disconnected", rt.ConnectionStatus())
}

func TestConfigRuntime_HandleSharedWSEvent_ConfigChanged(t *testing.T) {
	fetchCalled := false
	rt := &ConfigRuntime{
		configID:    "test-id",
		environment: "test",
		cache:       map[string]interface{}{"key": "old"},
		status:      "connected",
		closeCh:     make(chan struct{}),
		wsDone:      make(chan struct{}),
		fetchChain: func() ([]chainEntry, error) {
			fetchCalled = true
			return []chainEntry{{Values: map[string]interface{}{"key": "new"}}}, nil
		},
		apiKey: "test",
		wsBase: "ws://localhost:0",
	}

	rt.handleSharedWSEvent(map[string]interface{}{"event": "config_changed"})
	assert.True(t, fetchCalled)
}

func TestConfigRuntime_HandleSharedWSEvent_ConfigDeleted(t *testing.T) {
	rt := &ConfigRuntime{
		configID:    "test-id",
		environment: "test",
		cache:       map[string]interface{}{},
	...
	rt.handleSharedWSEvent(map[string]interface{}{"event": "config_deleted"})
	assert.Equal(t, "disconnected", rt.status)
}

func TestConfigRuntime_HandleSharedWSEvent_UnknownEvent(t *testing.T) {
	rt := &ConfigRuntime{
		configID:    "test-id",
		environment: "test",
		cache:       map[string]interface{}{},
	...
	// Unknown event should not panic or change status
	rt.handleSharedWSEvent(map[string]interface{}{"event": "unknown"})
	assert.Equal(t, "connected", rt.status)
}
